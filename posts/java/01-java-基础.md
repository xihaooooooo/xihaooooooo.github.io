...
动态代理的本质：**不修改原类代码，在运行时创建代理对象，在方法调用前后插入增强逻辑**。Java 生态主要有两种实现方式。

### 19.1 JDK 动态代理

基于接口，运行时由 `Proxy` 类动态生成一个实现了被代理接口的 `$ProxyX` 类。所有方法调用都会路由到 `InvocationHandler.invoke()`，在这里织入增强逻辑。

核心流程：实现 `InvocationHandler` 接口 → 覆写 `invoke()`（内部通过 `method.invoke(target, args)` 反射调用真实方法）→ 调用 `Proxy.newProxyInstance(classLoader, interfaces, handler)` 生成代理对象。

三个参数的含义：类加载器（加载动态生成的 class）、接口数组（告诉 Proxy 要生成哪些方法）、调用处理器（所有方法调用的入口）。

核心限制：**被代理对象必须实现至少一个接口**，因为生成的代理类本身已经是 Proxy 的子类（单继承），只能通过接口来代理。

### 19.2 CGLIB 动态代理

基于继承，底层用 ASM 字节码框架**动态生成目标类的子类**，重写父类方法，在重写方法中插入增强逻辑。

核心流程：实现 `MethodInterceptor` 接口 → 覆写 `intercept()`（内部通过 `proxy.invokeSuper()` 调用父类方法）→ 用 `Enhancer` 设置父类和回调，调用 `create()` 生成代理对象。

关键细节：`invokeSuper()` 不是反射调用，而是 ASM 生成的快速路径直接调用父类方法，性能优于 JDK 反射。限制：**不能代理 final 类和方法**（final 无法继承/重写），且创建代理对象时需要加载额外字节码，生成开销比 JDK 代理大。

### 19.3 Spring AOP 的代理策略

| 场景 | 使用的代理 |
|------|-----------|
| 目标类实现了接口 | JDK 动态代理（默认） |
| 目标类无接口 | CGLIB |
| Spring Boot 2.x+ | 默认全局 CGLIB |

Spring Boot 2.x 起默认 `spring.aop.proxy-target-class=true`，统一使用 CGLIB。原因：
- 避免强转时报 `ClassCastException`（JDK 代理对象不是实现类的实例，只是接口的实例）
- Bean 注入时按实现类类型注入的情况很常见，CGLIB 代理是子类，可以注入

如需强制切换：

```yaml
# application.yml — 切回 JDK 代理
spring:
  aop:
...
| 维度 | JDK 动态代理 | CGLIB |
|------|-------------|-------|
| 原理 | 基于接口，动态生成接口实现类 | 基于继承，动态生成子类 |
| 前提 | 必须有接口 | 不能是 final 类/方法 |
| 调用方式 | 反射 `Method.invoke()` | ASM 快速路径 `invokeSuper()` |
| 性能 | 生成快，执行略慢（反射） | 生成慢（ASM 操作字节码），执行快 |
| Spring 默认 | 老版本默认 | Boot 2.x+ 默认 |
...
一个类从 .class 文件到 JVM 可用，经历五个阶段：

- **加载**：通过全限定名找到字节码，把静态结构转换成方法区的运行时数据结构，同时在堆中生成 `Class` 对象作为访问入口。
- **验证**：检查 class 文件的正确性（魔数 CAFEBABE、版本号、元数据、字节码指令是否合法），防止恶意篡改的 class 文件破坏 JVM。
- **准备**：为静态变量分配内存并赋零值（`int`→0，引用类型→null）。注意：此时不会执行任何 Java 代码，`static final` 常量除外（编译期已确定值）。
- **解析**：把常量池中的符号引用（`#xxx`）替换为直接引用（内存地址），JVM 规范允许解析阶段延迟到真正使用时再执行（"迟解析"）。
- **初始化**：执行 `<clinit>` 方法，按代码顺序初始化静态变量和静态代码块。这是真正执行程序员写的初始化代码的阶段。

触发类初始化的时机（主动引用）：new 对象、访问静态变量/方法、反射、初始化子类时先初始化父类、Main 类启动。访问 `final static` 常量或定义数组引用不属于主动引用。

### 21.2 双亲委派模型

工作方式：**一个类加载器收到加载请求时，自己不先加载，而是逐层向上委托给父加载器**。只有父加载器反馈"无法加载"时，子加载器才尝试自己加载。

```
Bootstrap ClassLoader    ← C++ 实现，加载 rt.jar / java.*
    ↑
Platform ClassLoader     ← JDK 9+ 替换 Extension，加载 java.ext.* 等模块
    ↑
Application ClassLoader  ← 加载 classpath 下的用户类
    ↑
自定义 ClassLoader        ← 继承 ClassLoader，重写 findClass()
```

（JDK 8 及之前，第二层叫 Extension ClassLoader，加载 `jre/lib/ext/`；JDK 9 模块化后改名为 Platform ClassLoader。）

默认的 `loadClass()` 源码逻辑：`findLoadedClass()` 检查是否已加载 → 委托 `parent.loadClass()` → 父加载器加载失败再调用自己的 `findClass()`。

两个核心好处：
- **避免类的重复加载**：同一个类在各级加载器中只会被加载一次。
- **保护核心 API**：`java.lang.String` 等核心类由 Bootstrap 加载，用户自定义的同名类永远不会被加载，防止核心库被篡改。

### 21.3 打破双亲委派

打破的通用方式是**重写 `loadClass()` 而非 `findClass()`**（重写 `findClass()` 遵循双亲委派，因为 `loadClass` 的逻辑仍是先委派给父加载器）。

三个典型场景：

- **JDBC 4.0（SPI 机制）**：`DriverManager` 是核心库（Bootstrap 加载），但具体驱动（MySQL Driver 等）在 classpath 下。Bootstrap 看不见 classpath 的类，所以引入 **线程上下文类加载器**（`Thread.currentThread().getContextClassLoader()`），让 `DriverManager` 反向获取 Application ClassLoader 去加载驱动实现。

- **Tomcat WebappClassLoader**：Tomcat 需要隔离多个 Web 应用的类（A 应用的 lib 不能影响 B 应用），所以每个应用有独立的 WebappClassLoader。加载顺序故意反着来：先自己加载 → 找不到再委托父加载器。共享库（如 Tomcat 自身的 lib）走正常的双亲委派，保证全局唯一。

- **OSGi 模块化**：每个 Bundle 有自己的 ClassLoader，加载顺序是网状而非层级的——先从其他已暴露包的 Bundle 加载，找不到再从自己的 classpath 加载，最后才尝试父 ClassLoader。本质上是一张有向图，而不是一棵树。
...
`@interface` 是语法糖，编译后生成的实际上是一个**继承 `java.lang.annotation.Annotation` 的接口**。用 `javap -verbose` 反编译可以看到，自定义注解的 class 就是 `interface Xxx extends java.lang.annotation.Annotation`。

运行时通过反射获取注解时，JVM 会**动态生成一个代理对象**（`$Proxy` 类）来实现该注解接口，注解的成员方法返回的就是使用者写的值。也就是说，`getAnnotation()` 拿到的本质是一个 JDK 动态代理实例。

### 23.2 元注解详解

**`@Target`** — 限定注解能修饰的目标。取值为 `ElementType` 枚举：
...
| PACKAGE | 包 |
| TYPE_PARAMETER | 类型参数 `<T>`（JDK 8+） |
| TYPE_USE | 任何使用类型的地方（JDK 8+） |

不写 `@Target` 时默认所有位置都能用。

**`@Retention`** — 注解的生命周期：

- **SOURCE**：仅源码级别，编译后丢弃。例如 `@Override`、`@SuppressWarnings`。Lombok 的 `@Getter`/`@Setter` 也是这个级别——仅在编译时生成代码，不需要保留到运行时。
- **CLASS**：保留到 class 文件，但运行时不可通过反射读取。这是**默认值**，但实际项目中很少用默认，需要运行时处理的场景一律声明 RUNTIME。
- **RUNTIME**：运行时可通过反射读取。Spring 的 `@Autowired`、`@Transactional` 等都声明为此级别，因为它们在运行时被框架扫描和处理。

选择原则：只需要编译期处理的（编译检查、代码生成）→ SOURCE；需要运行时扫描的（IoC、AOP）→ RUNTIME；默认值 CLASS 实际很少用。

**`@Inherited`** — 标注后，子类会继承父类上的该注解。注意两个限制：
- 只对**类继承**有效，接口实现不继承注解
- 方法上的注解不继承（不标注 `@Inherited` 的方法注解本就通过 `@Override` 间接获取）

**`@Documented`** — 标记后，该注解会出现在 javadoc 生成的文档中。不加则文档中不显示。

**`@Repeatable`**（JDK 8+）— 允许同一个注解在同位置重复使用。需要定义一个"容器注解"来装多个重复注解。例如 Spring 的 `@PropertySource` 可以写多个：
...
编译器会自动把它们包装进 `@PropertySources` 容器中。

### 23.3 自定义注解与运行时处理

定义注解时，成员用**方法形式**声明，类型只能是基本类型、String、Class、枚举、注解及它们的数组。可以设默认值：
...
if (method.isAnnotationPresent(Log.class)) {
    ...
    // 根据 log.level() / log.async() 决定日志行为
    method.invoke(obj);  // 执行原方法
}
```

Spring AOP 的 `@Around` + 注解组合就是这种模式的经典应用——自定义一个业务注解，然后写一个 Aspect 拦截所有标注了该注解的方法。

### 23.4 编译期注解处理（APT）

JDK 提供了 `javax.annotation.processing.AbstractProcessor`，在编译时扫描和处理注解，可以生成新的 Java 源文件。Java 的 SPI（`META-INF/services`）会加载这些 Processor。

Lombok 是 APT 最激进的实践者——它直接操作 AST，在编译期插入 getter/setter 等代码。`@Getter` 声明为 SOURCE，编译后 class 文件里有 getter 方法但注解本身不见了。

MapStruct、Dagger 等工具也基于编译期注解处理生成代码，避免运行时反射的性能开销。

### 23.5 常见坑

- **注解不支持继承**：子接口不会自动继承父接口的注解，需要各自标注。这与类的 `@Inherited` 是两回事——`@Inherited` 只对类有效，对接口无效。
- **默认值陷阱**：注解成员如果没设默认值，使用时必须显式赋值，否则编译报错。设计注解时建议给常见值设置默认值。
- **反射获取的重复注解**：如果注解声明了 `@Repeatable`，用 `getAnnotation()` 拿容器，用 `getAnnotationsByType()` 拿单个类型更方便。
- **不要滥用注解**：注解适合做**配置性质的元数据**，不适合承载复杂业务逻辑。复杂的条件判断还是用代码更清晰。

---

## 24. 枚举 (Enum)

- **本质**：`enum` 编译后是 `final class Xxx extends java.lang.Enum`，枚举常量是 `public static final` 的类实例。
- **常用方法**：`values()`（所有常量数组）、`ordinal()`（序号）、`name()`（常量名）、`valueOf(String)`
- **不能继承**：因为编译后已经是 final 类，但可以实现接口。
- **枚举体**：可以有字段、构造器（私有的）、方法。适合做"固定选项"的数据建模。
- **枚举单例**：《Effective Java》推荐的最佳单例写法——天然防反射、防序列化破坏。`INSTANCE;` 一行搞定。
- **EnumSet / EnumMap**：专门为枚举优化的集合，底层用位向量/数组，性能极高。

---

## 25. Comparable vs Comparator

- **Comparable**：类内部实现 `compareTo()`，定义"自然顺序"。一次定义，随类绑定。`String`、`Integer` 都实现了 Comparable。
- **Comparator**：外部比较器，`compare(a, b)`。可以定义多个不同排序规则，不修改原类。常用 `Comparator.comparing()` 链式构建。
- **比较规则**：返回负数表示 o1 < o2，0 表示相等，正数表示 o1 > o2。
- **使用场景**：
  - `Collections.sort()` / `List.sort()` — 不传 Comparator 则要求元素实现 Comparable
  - `TreeMap` / `TreeSet` — 构造时可传 Comparator，否则依赖元素的自然顺序

---

## 26. fail-fast vs fail-safe

- **fail-fast**：迭代时如果集合被结构修改（add/remove），立即抛 `ConcurrentModificationException`。`ArrayList`、`HashMap` 等普通集合类都采用此策略。底层用 `modCount` 计数器检测——迭代器持有期望值，每次 next() 时比对，不一致就抛。
- **fail-safe**：迭代时操作的是集合的快照或副本，不会抛异常。`CopyOnWriteArrayList`、`ConcurrentHashMap` 采用此策略。
- **代价**：fail-safe 需要额外内存（拷贝），且迭代器不反映实时数据。
